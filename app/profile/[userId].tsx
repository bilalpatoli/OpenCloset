      ...
      {!isSelf && (
        <View style={styles.followRow}>
          <FollowButton
            ...
            onPress={() =>
              toggle().catch(() => Alert.alert('Error', 'Could not update follow status.'))
            }
          />
        </View>
      )}
...
function FilterChip({ label, count, active, onPress }: { label: string; count: number; active: boolean; onPress: () => void }) {
  return (
    <TouchableOpacity style={[styles.chip, active && styles.chipActive]} activeOpacity={0.8} onPress={onPress}>
      <Text style={[styles.chipText, active && styles.chipTextActive]}>{label}</Text>
      <Text style={[styles.chipCount, active && styles.chipCountActive]}>{String(count).padStart(2, '0')}</Text>
    </TouchableOpacity>
  );
}
...
const styles = StyleSheet.create({
  ...
  safeLoading: { flex: 1, backgroundColor: colors.background, alignItems: 'center', justifyContent: 'center' },
  ...
  avatarInitial: { fontFamily: typography.display, fontSize: 32, color: colors.text },
  statsContainer: { flex: 1, gap: spacing.sm },
  profileName: { fontFamily: typography.display, fontSize: 15, color: colors.text, letterSpacing: -0.3 },
  statsRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  statCol: { alignItems: 'flex-start', gap: 2 },
  statValue: { fontFamily: typography.display, fontSize: 18, color: colors.text, letterSpacing: -0.5 },
  statLabel: { fontFamily: typography.body, fontSize: 10, color: colors.textSecondary },
  profileMeta: { paddingHorizontal: spacing.lg, paddingTop: spacing.xs, paddingBottom: spacing.sm, gap: spacing.sm },
  bioText: { fontFamily: typography.body, fontSize: 13, color: colors.text, lineHeight: 18 },
  locationRow: { flexDirection: 'row', alignItems: 'center', gap: spacing.xs },
  locationText: { fontFamily: typography.body, fontSize: 12, color: colors.textSecondary },
  followRow: { paddingHorizontal: spacing.lg, paddingTop: spacing.sm, paddingBottom: spacing.xs },
  tabBar: { flexDirection: 'row', borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: colors.border, marginTop: spacing.md },
  tabItem: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: spacing.xs, paddingVertical: spacing.md, borderBottomWidth: 1.5, borderBottomColor: 'transparent' },
  tabItemActive: { borderBottomColor: colors.text },
  tabLabel: { fontFamily: typography.body, fontSize: 9.5, letterSpacing: 1.2, textTransform: 'uppercase', color: colors.textTertiary, fontWeight: '600' },
  tabLabelActive: { color: colors.text },
  filterRow: { paddingHorizontal: spacing.lg, paddingTop: spacing.md, paddingBottom: spacing.sm, gap: spacing.sm },
  chip: { flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 14, paddingVertical: 8, borderRadius: radius.pill, backgroundColor: colors.white, borderWidth: StyleSheet.hairlineWidth, borderColor: colors.border },
  chipActive: { backgroundColor: colors.text, borderColor: colors.text },
  chipText: { fontFamily: typography.body, fontSize: 11, letterSpacing: 1.2, textTransform: 'uppercase', color: colors.text, fontWeight: '600' },
  chipTextActive: { color: colors.white },
  chipCount: { fontFamily: typography.serif, fontStyle: 'italic', fontSize: 11, color: colors.accent },
  ...
  wardrobeFallback: { width: '100%', aspectRatio: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.surface },
  wardrobeName: { fontFamily: typography.body, fontSize: 10.5, color: colors.textSecondary, letterSpacing: 0.3 },
  emptyCard: { marginTop: spacing.xl, marginHorizontal: spacing.lg, paddingVertical: spacing.xxxl, borderRadius: radius.lg, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.surface, gap: spacing.md },
  emptyText: { fontFamily: typography.serif, fontStyle: 'italic', fontSize: 14, color: colors.textSecondary },
});
